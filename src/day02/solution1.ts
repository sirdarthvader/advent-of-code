 ...
 * 
 * Solution: 
 * - Read and Parse the input into a list of reports, where each report is an array of numbers.
 * - Check Each Report: For each report:
 *  - Determine if the report is strictly increasing or strictly decreasing.
 *  - Verify that the differences between adjacent numbers are within the range [1, 3].
  * - Count the Safe Reports: Sum up the number of reports that meet both conditions.
...
function countSafeReports(reports: string[]): number {
  ...
  for (const reportStr of reports) {
    const report = reportStr.trim().split(" ").map(Number); // Convert to an array of numbers

    if (isSafeReport(report)) {
      safeCount++; // Increment count if report is safe
    }
  }
...
console.log("Safe Reports bla " + safeReports);
